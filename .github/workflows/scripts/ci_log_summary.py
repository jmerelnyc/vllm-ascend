...
def _fetch_pr_test_full_vllm_commit(ref: str) -> str | None:
    ...
    try:
        workflow_data = gh_api_json(
            "/repos/user/vllm-ascend/contents/.github/workflows/pr_test_full.yaml",
            ref=ref,
        )
...
def extract_bad_commit(log_text: str, *, resolve_remote: bool = True) -> str | None:
    ...
    checkout_vllm_repo_re = re.compile(r"^\S+\s+repository:\s+user/vllm\s*$")
    ...
    uses_match = re.search(
        r"Uses:\s+user/vllm-ascend/\.github/workflows/_e2e_test\.yaml@(\S+)\s+\(([0-9a-f]{40})\)",
        log_text,
    )
    if uses_match:
