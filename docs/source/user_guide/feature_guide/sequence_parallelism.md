...
FC1 is a unique optimization in vllm-ascend, currently implemented based on Custom OP, but it is difficult to support VL-class models (reasons detailed in [[RFC]: support sequence parallelism by pass](https://github.com/user/vllm-ascend/issues/5712) ). Therefore, currently FC1 and SP are complementary.
