...
As introduced in the [RFC](https://github.com/user/vllm-ascend/issues/4715), Npugraph_ex is a compile-time FX graph optimization layer that works together with ACLGraph. It optimizes the model's FX graph before ACLGraph captures it at runtime. Its performance benefits mainly come from fusing multiple operators into single kernels (e.g., add + rms_norm → npu_add_rms_norm) to reduce kernel launch overhead.
...
- [Npugraph_ex RFC](https://github.com/user/vllm-ascend/issues/4715)
