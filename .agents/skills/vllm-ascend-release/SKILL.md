...
- GitHub CLI (`gh`) authenticated with write access to `user/vllm-ascend`
...
gh release list --repo user/vllm-ascend --limit 5
...
```bash
gh issue create --repo user/vllm-ascend \
...
gh pr list --repo user/vllm-ascend \
  --state open \
  --search "milestone:${VERSION}" \
  --json number,title,url,labels

gh issue list --repo user/vllm-ascend \
  --state open \
  --search "milestone:${VERSION}" \
  --json number,title,url,labels
...
- Look up PR descriptions when uncertain: `gh pr view <number> --repo user/vllm-ascend`
- Group related changes together
- Include PR links: `[#12345](https://github.com/user/vllm-ascend/pull/12345)`
...
# Create PR
gh pr create --repo user/vllm-ascend \
...
gh pr merge ${RELEASE_PR_NUMBER} --repo user/vllm-ascend --squash
...
➡️ 源码地址：https://github.com/user/vllm-ascend/releases/tag/${VERSION}
...
gh release view ${VERSION} --repo user/vllm-ascend --json body,name,tagName
...
➡️ 源码地址：
https://github.com/user/vllm-ascend/releases/tag/v0.18.0rc1
➡️ 快速体验：
...
**Arguments:**
- `--owner`: Repository owner (default: user)
...
**Arguments:**
- `--repo`: Repository (default: user/vllm-ascend)
